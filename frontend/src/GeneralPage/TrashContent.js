...
const TrashContent = ({ emails }) => {
  ...
  const handelRestore = async (e) => {
    e.preventDefault();

    await fetch(`http://localhost:8080/api/mails/${emails[selectedEmail].id}/restore`, {
            method: 'PUT',
            headers: {
            ...
            window.alert("Udało się przywrócić maila!");
        ...
        {selectedEmail !== null ? (
          <div className="email-details-content">
            ...
            <div className='form-buttons'>
              <button type="button" onClick={handelRestore}>Przywróć</button>
            </div>
          </div>
        ) : (
